using System;
using System.Linq;
using System.Numerics;
using Xunit;

namespace SmartVectorDotNet;


public partial class VectorizationTest
{
    public static TheoryData<int[][]> TestCaseIntegerVectorization()
    {
        static int[][] createTestData(int seed, int size)
        {
            var rand = new Random(seed);
            var arguments = new int[10][];
            for(var i = 0; i < 10; ++i)
            {
                arguments[i] = Enumerable.Range(0, size).Select(_ => rand.Next()).ToArray();
            }
            return arguments;
        }

        return new TheoryData<int[][]>()
        {
            { createTestData(12345678 + 0, 0) },
            { createTestData(12345678 + 1, 1) },
            { createTestData(12345678 + 3, 3) },
            { createTestData(12345678 + 4, 4) },
            { createTestData(12345678 + 5, 5) },
            { createTestData(12345678 + 7, 7) },
            { createTestData(12345678 + 8, 8) },
            { createTestData(12345678 + 9, 9) },
            { createTestData(12345678 + 15, 15) },
            { createTestData(12345678 + 16, 16) },
            { createTestData(12345678 + 17, 17) },
            { createTestData(12345678 + 31, 31) },
            { createTestData(12345678 + 32, 32) },
            { createTestData(12345678 + 33, 33) },
            { createTestData(12345678 + 63, 63) },
            { createTestData(12345678 + 64, 64) },
            { createTestData(12345678 + 65, 65) },
            { createTestData(12345678 + 127, 127) },
            { createTestData(12345678 + 128, 128) },
            { createTestData(12345678 + 129, 129) },
            { createTestData(12345678 + 255, 255) },
            { createTestData(12345678 + 256, 256) },
            { createTestData(12345678 + 257, 257) },
        };
    }


    private readonly struct TestIntegerOp1 : IVectorFormula1<int>
    {
        public int Calculate(int x1)
            => x1 / x1;
            
        public Vector<int> Calculate(Vector<int> x1)
            => x1 / x1;
    }

    [Theory]
    [MemberData(nameof(TestCaseIntegerVectorization))]
    public void CalculateIntegerVector1(int[][] arguments)
    {
        var op = default(TestIntegerOp1);
        AssertEx.SameBehavior(
            () =>
            {
                var exp = new int[arguments[0].Length];
                for(var i = 0; i < exp.Length; ++i)
                {
                    exp[i] = op.Calculate(arguments[0][i]);
                }
                return exp;
            },
            () =>
            {
                var act = new int[arguments[0].Length];
                Vectorization.SIMD.Calculate<int, TestIntegerOp1>(arguments[0], act);
                return act;
            });
    }

    private readonly struct TestIntegerOp2 : IVectorFormula2<int>
    {
        public int Calculate(int x1, int x2)
            => x1 + x2 / x1;
            
        public Vector<int> Calculate(Vector<int> x1, Vector<int> x2)
            => x1 + x2 / x1;
    }

    [Theory]
    [MemberData(nameof(TestCaseIntegerVectorization))]
    public void CalculateIntegerVector2(int[][] arguments)
    {
        var op = default(TestIntegerOp2);
        AssertEx.SameBehavior(
            () =>
            {
                var exp = new int[arguments[0].Length];
                for(var i = 0; i < exp.Length; ++i)
                {
                    exp[i] = op.Calculate(arguments[0][i], arguments[1][i]);
                }
                return exp;
            },
            () =>
            {
                var act = new int[arguments[0].Length];
                Vectorization.SIMD.Calculate<int, TestIntegerOp2>(arguments[0], arguments[1], act);
                return act;
            });
    }

    private readonly struct TestIntegerOp3 : IVectorFormula3<int>
    {
        public int Calculate(int x1, int x2, int x3)
            => x1 + x2 + x3 / x1;
            
        public Vector<int> Calculate(Vector<int> x1, Vector<int> x2, Vector<int> x3)
            => x1 + x2 + x3 / x1;
    }

    [Theory]
    [MemberData(nameof(TestCaseIntegerVectorization))]
    public void CalculateIntegerVector3(int[][] arguments)
    {
        var op = default(TestIntegerOp3);
        AssertEx.SameBehavior(
            () =>
            {
                var exp = new int[arguments[0].Length];
                for(var i = 0; i < exp.Length; ++i)
                {
                    exp[i] = op.Calculate(arguments[0][i], arguments[1][i], arguments[2][i]);
                }
                return exp;
            },
            () =>
            {
                var act = new int[arguments[0].Length];
                Vectorization.SIMD.Calculate<int, TestIntegerOp3>(arguments[0], arguments[1], arguments[2], act);
                return act;
            });
    }

    private readonly struct TestIntegerOp4 : IVectorFormula4<int>
    {
        public int Calculate(int x1, int x2, int x3, int x4)
            => x1 + x2 + x3 + x4 / x1;
            
        public Vector<int> Calculate(Vector<int> x1, Vector<int> x2, Vector<int> x3, Vector<int> x4)
            => x1 + x2 + x3 + x4 / x1;
    }

    [Theory]
    [MemberData(nameof(TestCaseIntegerVectorization))]
    public void CalculateIntegerVector4(int[][] arguments)
    {
        var op = default(TestIntegerOp4);
        AssertEx.SameBehavior(
            () =>
            {
                var exp = new int[arguments[0].Length];
                for(var i = 0; i < exp.Length; ++i)
                {
                    exp[i] = op.Calculate(arguments[0][i], arguments[1][i], arguments[2][i], arguments[3][i]);
                }
                return exp;
            },
            () =>
            {
                var act = new int[arguments[0].Length];
                Vectorization.SIMD.Calculate<int, TestIntegerOp4>(arguments[0], arguments[1], arguments[2], arguments[3], act);
                return act;
            });
    }

    private readonly struct TestIntegerOp5 : IVectorFormula5<int>
    {
        public int Calculate(int x1, int x2, int x3, int x4, int x5)
            => x1 + x2 + x3 + x4 + x5 / x1;
            
        public Vector<int> Calculate(Vector<int> x1, Vector<int> x2, Vector<int> x3, Vector<int> x4, Vector<int> x5)
            => x1 + x2 + x3 + x4 + x5 / x1;
    }

    [Theory]
    [MemberData(nameof(TestCaseIntegerVectorization))]
    public void CalculateIntegerVector5(int[][] arguments)
    {
        var op = default(TestIntegerOp5);
        AssertEx.SameBehavior(
            () =>
            {
                var exp = new int[arguments[0].Length];
                for(var i = 0; i < exp.Length; ++i)
                {
                    exp[i] = op.Calculate(arguments[0][i], arguments[1][i], arguments[2][i], arguments[3][i], arguments[4][i]);
                }
                return exp;
            },
            () =>
            {
                var act = new int[arguments[0].Length];
                Vectorization.SIMD.Calculate<int, TestIntegerOp5>(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4], act);
                return act;
            });
    }

    private readonly struct TestIntegerOp6 : IVectorFormula6<int>
    {
        public int Calculate(int x1, int x2, int x3, int x4, int x5, int x6)
            => x1 + x2 + x3 + x4 + x5 + x6 / x1;
            
        public Vector<int> Calculate(Vector<int> x1, Vector<int> x2, Vector<int> x3, Vector<int> x4, Vector<int> x5, Vector<int> x6)
            => x1 + x2 + x3 + x4 + x5 + x6 / x1;
    }

    [Theory]
    [MemberData(nameof(TestCaseIntegerVectorization))]
    public void CalculateIntegerVector6(int[][] arguments)
    {
        var op = default(TestIntegerOp6);
        AssertEx.SameBehavior(
            () =>
            {
                var exp = new int[arguments[0].Length];
                for(var i = 0; i < exp.Length; ++i)
                {
                    exp[i] = op.Calculate(arguments[0][i], arguments[1][i], arguments[2][i], arguments[3][i], arguments[4][i], arguments[5][i]);
                }
                return exp;
            },
            () =>
            {
                var act = new int[arguments[0].Length];
                Vectorization.SIMD.Calculate<int, TestIntegerOp6>(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4], arguments[5], act);
                return act;
            });
    }

    private readonly struct TestIntegerOp7 : IVectorFormula7<int>
    {
        public int Calculate(int x1, int x2, int x3, int x4, int x5, int x6, int x7)
            => x1 + x2 + x3 + x4 + x5 + x6 + x7 / x1;
            
        public Vector<int> Calculate(Vector<int> x1, Vector<int> x2, Vector<int> x3, Vector<int> x4, Vector<int> x5, Vector<int> x6, Vector<int> x7)
            => x1 + x2 + x3 + x4 + x5 + x6 + x7 / x1;
    }

    [Theory]
    [MemberData(nameof(TestCaseIntegerVectorization))]
    public void CalculateIntegerVector7(int[][] arguments)
    {
        var op = default(TestIntegerOp7);
        AssertEx.SameBehavior(
            () =>
            {
                var exp = new int[arguments[0].Length];
                for(var i = 0; i < exp.Length; ++i)
                {
                    exp[i] = op.Calculate(arguments[0][i], arguments[1][i], arguments[2][i], arguments[3][i], arguments[4][i], arguments[5][i], arguments[6][i]);
                }
                return exp;
            },
            () =>
            {
                var act = new int[arguments[0].Length];
                Vectorization.SIMD.Calculate<int, TestIntegerOp7>(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4], arguments[5], arguments[6], act);
                return act;
            });
    }

    private readonly struct TestIntegerOp8 : IVectorFormula8<int>
    {
        public int Calculate(int x1, int x2, int x3, int x4, int x5, int x6, int x7, int x8)
            => x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 / x1;
            
        public Vector<int> Calculate(Vector<int> x1, Vector<int> x2, Vector<int> x3, Vector<int> x4, Vector<int> x5, Vector<int> x6, Vector<int> x7, Vector<int> x8)
            => x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 / x1;
    }

    [Theory]
    [MemberData(nameof(TestCaseIntegerVectorization))]
    public void CalculateIntegerVector8(int[][] arguments)
    {
        var op = default(TestIntegerOp8);
        AssertEx.SameBehavior(
            () =>
            {
                var exp = new int[arguments[0].Length];
                for(var i = 0; i < exp.Length; ++i)
                {
                    exp[i] = op.Calculate(arguments[0][i], arguments[1][i], arguments[2][i], arguments[3][i], arguments[4][i], arguments[5][i], arguments[6][i], arguments[7][i]);
                }
                return exp;
            },
            () =>
            {
                var act = new int[arguments[0].Length];
                Vectorization.SIMD.Calculate<int, TestIntegerOp8>(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4], arguments[5], arguments[6], arguments[7], act);
                return act;
            });
    }

    private readonly struct TestIntegerOp9 : IVectorFormula9<int>
    {
        public int Calculate(int x1, int x2, int x3, int x4, int x5, int x6, int x7, int x8, int x9)
            => x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 / x1;
            
        public Vector<int> Calculate(Vector<int> x1, Vector<int> x2, Vector<int> x3, Vector<int> x4, Vector<int> x5, Vector<int> x6, Vector<int> x7, Vector<int> x8, Vector<int> x9)
            => x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 / x1;
    }

    [Theory]
    [MemberData(nameof(TestCaseIntegerVectorization))]
    public void CalculateIntegerVector9(int[][] arguments)
    {
        var op = default(TestIntegerOp9);
        AssertEx.SameBehavior(
            () =>
            {
                var exp = new int[arguments[0].Length];
                for(var i = 0; i < exp.Length; ++i)
                {
                    exp[i] = op.Calculate(arguments[0][i], arguments[1][i], arguments[2][i], arguments[3][i], arguments[4][i], arguments[5][i], arguments[6][i], arguments[7][i], arguments[8][i]);
                }
                return exp;
            },
            () =>
            {
                var act = new int[arguments[0].Length];
                Vectorization.SIMD.Calculate<int, TestIntegerOp9>(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4], arguments[5], arguments[6], arguments[7], arguments[8], act);
                return act;
            });
    }

    private readonly struct TestIntegerOp10 : IVectorFormula10<int>
    {
        public int Calculate(int x1, int x2, int x3, int x4, int x5, int x6, int x7, int x8, int x9, int x10)
            => x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 + x10 / x1;
            
        public Vector<int> Calculate(Vector<int> x1, Vector<int> x2, Vector<int> x3, Vector<int> x4, Vector<int> x5, Vector<int> x6, Vector<int> x7, Vector<int> x8, Vector<int> x9, Vector<int> x10)
            => x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 + x10 / x1;
    }

    [Theory]
    [MemberData(nameof(TestCaseIntegerVectorization))]
    public void CalculateIntegerVector10(int[][] arguments)
    {
        var op = default(TestIntegerOp10);
        AssertEx.SameBehavior(
            () =>
            {
                var exp = new int[arguments[0].Length];
                for(var i = 0; i < exp.Length; ++i)
                {
                    exp[i] = op.Calculate(arguments[0][i], arguments[1][i], arguments[2][i], arguments[3][i], arguments[4][i], arguments[5][i], arguments[6][i], arguments[7][i], arguments[8][i], arguments[9][i]);
                }
                return exp;
            },
            () =>
            {
                var act = new int[arguments[0].Length];
                Vectorization.SIMD.Calculate<int, TestIntegerOp10>(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4], arguments[5], arguments[6], arguments[7], arguments[8], arguments[9], act);
                return act;
            });
    }

}


public partial class VectorizationTest
{
    public static TheoryData<float[][]> TestCaseFloatVectorization()
    {
        static float[][] createTestData(int seed, int size)
        {
            var rand = new Random(seed);
            var arguments = new float[10][];
            for(var i = 0; i < 10; ++i)
            {
                arguments[i] = Enumerable.Range(0, size).Select(_ => (float)rand.NextDouble()).ToArray();
            }
            return arguments;
        }

        return new TheoryData<float[][]>()
        {
            { createTestData(12345678 + 0, 0) },
            { createTestData(12345678 + 1, 1) },
            { createTestData(12345678 + 3, 3) },
            { createTestData(12345678 + 4, 4) },
            { createTestData(12345678 + 5, 5) },
            { createTestData(12345678 + 7, 7) },
            { createTestData(12345678 + 8, 8) },
            { createTestData(12345678 + 9, 9) },
            { createTestData(12345678 + 15, 15) },
            { createTestData(12345678 + 16, 16) },
            { createTestData(12345678 + 17, 17) },
            { createTestData(12345678 + 31, 31) },
            { createTestData(12345678 + 32, 32) },
            { createTestData(12345678 + 33, 33) },
            { createTestData(12345678 + 63, 63) },
            { createTestData(12345678 + 64, 64) },
            { createTestData(12345678 + 65, 65) },
            { createTestData(12345678 + 127, 127) },
            { createTestData(12345678 + 128, 128) },
            { createTestData(12345678 + 129, 129) },
            { createTestData(12345678 + 255, 255) },
            { createTestData(12345678 + 256, 256) },
            { createTestData(12345678 + 257, 257) },
        };
    }


    private readonly struct TestFloatOp1 : IVectorFormula1<float>
    {
        public float Calculate(float x1)
            => x1;
            
        public Vector<float> Calculate(Vector<float> x1)
            => x1;
    }

    [Theory]
    [MemberData(nameof(TestCaseFloatVectorization))]
    public void CalculateFloatVector1(float[][] arguments)
    {
        var op = default(TestFloatOp1);
        var exp = new float[arguments[0].Length];
        var act = new float[arguments[0].Length];
        for(var i = 0; i < exp.Length; ++i)
        {
            exp[i] = op.Calculate(arguments[0][i]);
        }
        Vectorization.SIMD.Calculate<float, TestFloatOp1>(arguments[0], act);
        Assert.Equal(exp, act);
    }

    private readonly struct TestFloatOp2 : IVectorFormula2<float>
    {
        public float Calculate(float x1, float x2)
            => x1 + x2;
            
        public Vector<float> Calculate(Vector<float> x1, Vector<float> x2)
            => x1 + x2;
    }

    [Theory]
    [MemberData(nameof(TestCaseFloatVectorization))]
    public void CalculateFloatVector2(float[][] arguments)
    {
        var op = default(TestFloatOp2);
        var exp = new float[arguments[0].Length];
        var act = new float[arguments[0].Length];
        for(var i = 0; i < exp.Length; ++i)
        {
            exp[i] = op.Calculate(arguments[0][i], arguments[1][i]);
        }
        Vectorization.SIMD.Calculate<float, TestFloatOp2>(arguments[0], arguments[1], act);
        Assert.Equal(exp, act);
    }

    private readonly struct TestFloatOp3 : IVectorFormula3<float>
    {
        public float Calculate(float x1, float x2, float x3)
            => x1 + x2 + x3;
            
        public Vector<float> Calculate(Vector<float> x1, Vector<float> x2, Vector<float> x3)
            => x1 + x2 + x3;
    }

    [Theory]
    [MemberData(nameof(TestCaseFloatVectorization))]
    public void CalculateFloatVector3(float[][] arguments)
    {
        var op = default(TestFloatOp3);
        var exp = new float[arguments[0].Length];
        var act = new float[arguments[0].Length];
        for(var i = 0; i < exp.Length; ++i)
        {
            exp[i] = op.Calculate(arguments[0][i], arguments[1][i], arguments[2][i]);
        }
        Vectorization.SIMD.Calculate<float, TestFloatOp3>(arguments[0], arguments[1], arguments[2], act);
        Assert.Equal(exp, act);
    }

    private readonly struct TestFloatOp4 : IVectorFormula4<float>
    {
        public float Calculate(float x1, float x2, float x3, float x4)
            => x1 + x2 + x3 + x4;
            
        public Vector<float> Calculate(Vector<float> x1, Vector<float> x2, Vector<float> x3, Vector<float> x4)
            => x1 + x2 + x3 + x4;
    }

    [Theory]
    [MemberData(nameof(TestCaseFloatVectorization))]
    public void CalculateFloatVector4(float[][] arguments)
    {
        var op = default(TestFloatOp4);
        var exp = new float[arguments[0].Length];
        var act = new float[arguments[0].Length];
        for(var i = 0; i < exp.Length; ++i)
        {
            exp[i] = op.Calculate(arguments[0][i], arguments[1][i], arguments[2][i], arguments[3][i]);
        }
        Vectorization.SIMD.Calculate<float, TestFloatOp4>(arguments[0], arguments[1], arguments[2], arguments[3], act);
        Assert.Equal(exp, act);
    }

    private readonly struct TestFloatOp5 : IVectorFormula5<float>
    {
        public float Calculate(float x1, float x2, float x3, float x4, float x5)
            => x1 + x2 + x3 + x4 + x5;
            
        public Vector<float> Calculate(Vector<float> x1, Vector<float> x2, Vector<float> x3, Vector<float> x4, Vector<float> x5)
            => x1 + x2 + x3 + x4 + x5;
    }

    [Theory]
    [MemberData(nameof(TestCaseFloatVectorization))]
    public void CalculateFloatVector5(float[][] arguments)
    {
        var op = default(TestFloatOp5);
        var exp = new float[arguments[0].Length];
        var act = new float[arguments[0].Length];
        for(var i = 0; i < exp.Length; ++i)
        {
            exp[i] = op.Calculate(arguments[0][i], arguments[1][i], arguments[2][i], arguments[3][i], arguments[4][i]);
        }
        Vectorization.SIMD.Calculate<float, TestFloatOp5>(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4], act);
        Assert.Equal(exp, act);
    }

    private readonly struct TestFloatOp6 : IVectorFormula6<float>
    {
        public float Calculate(float x1, float x2, float x3, float x4, float x5, float x6)
            => x1 + x2 + x3 + x4 + x5 + x6;
            
        public Vector<float> Calculate(Vector<float> x1, Vector<float> x2, Vector<float> x3, Vector<float> x4, Vector<float> x5, Vector<float> x6)
            => x1 + x2 + x3 + x4 + x5 + x6;
    }

    [Theory]
    [MemberData(nameof(TestCaseFloatVectorization))]
    public void CalculateFloatVector6(float[][] arguments)
    {
        var op = default(TestFloatOp6);
        var exp = new float[arguments[0].Length];
        var act = new float[arguments[0].Length];
        for(var i = 0; i < exp.Length; ++i)
        {
            exp[i] = op.Calculate(arguments[0][i], arguments[1][i], arguments[2][i], arguments[3][i], arguments[4][i], arguments[5][i]);
        }
        Vectorization.SIMD.Calculate<float, TestFloatOp6>(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4], arguments[5], act);
        Assert.Equal(exp, act);
    }

    private readonly struct TestFloatOp7 : IVectorFormula7<float>
    {
        public float Calculate(float x1, float x2, float x3, float x4, float x5, float x6, float x7)
            => x1 + x2 + x3 + x4 + x5 + x6 + x7;
            
        public Vector<float> Calculate(Vector<float> x1, Vector<float> x2, Vector<float> x3, Vector<float> x4, Vector<float> x5, Vector<float> x6, Vector<float> x7)
            => x1 + x2 + x3 + x4 + x5 + x6 + x7;
    }

    [Theory]
    [MemberData(nameof(TestCaseFloatVectorization))]
    public void CalculateFloatVector7(float[][] arguments)
    {
        var op = default(TestFloatOp7);
        var exp = new float[arguments[0].Length];
        var act = new float[arguments[0].Length];
        for(var i = 0; i < exp.Length; ++i)
        {
            exp[i] = op.Calculate(arguments[0][i], arguments[1][i], arguments[2][i], arguments[3][i], arguments[4][i], arguments[5][i], arguments[6][i]);
        }
        Vectorization.SIMD.Calculate<float, TestFloatOp7>(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4], arguments[5], arguments[6], act);
        Assert.Equal(exp, act);
    }

    private readonly struct TestFloatOp8 : IVectorFormula8<float>
    {
        public float Calculate(float x1, float x2, float x3, float x4, float x5, float x6, float x7, float x8)
            => x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8;
            
        public Vector<float> Calculate(Vector<float> x1, Vector<float> x2, Vector<float> x3, Vector<float> x4, Vector<float> x5, Vector<float> x6, Vector<float> x7, Vector<float> x8)
            => x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8;
    }

    [Theory]
    [MemberData(nameof(TestCaseFloatVectorization))]
    public void CalculateFloatVector8(float[][] arguments)
    {
        var op = default(TestFloatOp8);
        var exp = new float[arguments[0].Length];
        var act = new float[arguments[0].Length];
        for(var i = 0; i < exp.Length; ++i)
        {
            exp[i] = op.Calculate(arguments[0][i], arguments[1][i], arguments[2][i], arguments[3][i], arguments[4][i], arguments[5][i], arguments[6][i], arguments[7][i]);
        }
        Vectorization.SIMD.Calculate<float, TestFloatOp8>(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4], arguments[5], arguments[6], arguments[7], act);
        Assert.Equal(exp, act);
    }

    private readonly struct TestFloatOp9 : IVectorFormula9<float>
    {
        public float Calculate(float x1, float x2, float x3, float x4, float x5, float x6, float x7, float x8, float x9)
            => x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9;
            
        public Vector<float> Calculate(Vector<float> x1, Vector<float> x2, Vector<float> x3, Vector<float> x4, Vector<float> x5, Vector<float> x6, Vector<float> x7, Vector<float> x8, Vector<float> x9)
            => x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9;
    }

    [Theory]
    [MemberData(nameof(TestCaseFloatVectorization))]
    public void CalculateFloatVector9(float[][] arguments)
    {
        var op = default(TestFloatOp9);
        var exp = new float[arguments[0].Length];
        var act = new float[arguments[0].Length];
        for(var i = 0; i < exp.Length; ++i)
        {
            exp[i] = op.Calculate(arguments[0][i], arguments[1][i], arguments[2][i], arguments[3][i], arguments[4][i], arguments[5][i], arguments[6][i], arguments[7][i], arguments[8][i]);
        }
        Vectorization.SIMD.Calculate<float, TestFloatOp9>(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4], arguments[5], arguments[6], arguments[7], arguments[8], act);
        Assert.Equal(exp, act);
    }

    private readonly struct TestFloatOp10 : IVectorFormula10<float>
    {
        public float Calculate(float x1, float x2, float x3, float x4, float x5, float x6, float x7, float x8, float x9, float x10)
            => x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 + x10;
            
        public Vector<float> Calculate(Vector<float> x1, Vector<float> x2, Vector<float> x3, Vector<float> x4, Vector<float> x5, Vector<float> x6, Vector<float> x7, Vector<float> x8, Vector<float> x9, Vector<float> x10)
            => x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 + x10;
    }

    [Theory]
    [MemberData(nameof(TestCaseFloatVectorization))]
    public void CalculateFloatVector10(float[][] arguments)
    {
        var op = default(TestFloatOp10);
        var exp = new float[arguments[0].Length];
        var act = new float[arguments[0].Length];
        for(var i = 0; i < exp.Length; ++i)
        {
            exp[i] = op.Calculate(arguments[0][i], arguments[1][i], arguments[2][i], arguments[3][i], arguments[4][i], arguments[5][i], arguments[6][i], arguments[7][i], arguments[8][i], arguments[9][i]);
        }
        Vectorization.SIMD.Calculate<float, TestFloatOp10>(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4], arguments[5], arguments[6], arguments[7], arguments[8], arguments[9], act);
        Assert.Equal(exp, act);
    }

}
